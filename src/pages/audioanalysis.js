...
const Analysis = () => {
    ...
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0eXBlIjoiSW50ZWdyYXRpb25BY2Nlc3NUb2tlbiIsInZlcnNpb24iOiIxLjAiLCJpbnRlZ3JhdGlvbklkIjoxNDA2LCJ1c2VySWQiOjk3MTIzLCJhY2Nlc3NUb2tlblNlY3JldCI6IjgyMTNjZmM4YTUyZDQ2NWEzNmJkOThmZWZhYTRkZTczMDBiZjAwOWJhYjg0NWU1M2M5ZDZiNzM2MzYzNzk2ODYiLCJpYXQiOjE3NDE0NTc3NDN9.2ao7kVgk1u5_y0MRs9ta_KKZhTvOpwpjWGq8RIbZJgI";
    ...
    const handleCompare = async () => {
        ...
        try {
            ...
            const similarTrack = similarityCheckData.data.libraryTrack.similarTracks.edges.find(
                (edge) => edge.node.id === newTrackIds[1]
            );

            const analysisResults = await Promise.all(newTrackIds.map(fetchSongAnalysis));
            setAnalysisResults(analysisResults.map(data => data.data.libraryTrack.audioAnalysisV6.result));
    ...
    const generateWaveform = () => {
        return Array.from({ length: 20 }, (_, i) => {
            ...
            return <div key={i} className="wave-bar" style={{ height: `${height}px` }}></div>;
        });
    };
    ...
    return (
        <div className="container">
                ...
                <div className="table-container">
                    ...
                    <div className="table-row-container">
                        <div className="table-row">
                            ...
                            <div className="value">{analysisResults[0]?.bpmRangeAdjusted || "N/A"}</div>
                            <div className="value">{analysisResults[1]?.bpmRangeAdjusted || "N/A"}</div>
                        </div>
                        ...
                        <div className="table-row">
                            ...
                            <div className="value">{displayTags(analysisResults[0]?.voiceTags)}</div>
                            <div className="value">{displayTags(analysisResults[1]?.voiceTags)}</div>
                        </div>
                        
                        <div className="table-row">
                            <div className="property">Energy</div>
                            <div className="value">{analysisResults[0]?.energyDynamics || "N/A"}</div>
                            <div className="value">{analysisResults[1]?.energyDynamics || "N/A"}</div>
                        </div>
                        
                        <div className="table-row">
                            <div className="property">Genre</div>
                            <div className="value">{displayTags(analysisResults[0]?.genreTags)}</div>
                            <div className="value">{displayTags(analysisResults[1]?.genreTags)}</div>
                        </div>
                        
                        <div className="table-row">
                            <div className="property">Subgenre</div>
                            <div className="value">{displayTags(analysisResults[0]?.subgenreTags)}</div>
                            <div className="value">{displayTags(analysisResults[1]?.subgenreTags)}</div>
                        </div>
                        
                        <div className="table-row">
                            <div className="property">Mood</div>
                            <div className="value">{displayTags(analysisResults[0]?.moodTags)}</div>
                            <div className="value">{displayTags(analysisResults[1]?.moodTags)}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="segment-btn-container">
                <button className="segment-btn">🎵 Play Most Representative Segments</button>
            </div>
